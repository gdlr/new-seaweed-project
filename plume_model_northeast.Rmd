...
points <- read_sf(here("new_data", "GIS_output", "nitrogen"), layer = "northeast_snapjoin") %>%
  select("CumAreaKm2", "tn", "geometry")
...
```{r}
path <- here("plume_outputs", "northeast_tn")

plume_fx <- function(points, raster, c = 0.5, b = 0.65, path){
  
  ####
  ####  Before running: 
  ####  Change CumAreaKm2 and test_pt$tn to match the data frame
...
phosphorous <- read_sf(here("new_data", "GIS_output", "phosphorous"), layer = "northeast_snapjoinP") %>%
  select("CumAreaKm2", "tp", "geometry")
pathP <- here("plume_outputs", "northeast_tp")

plume_fx(phosphorous, ocean, path = pathP)
